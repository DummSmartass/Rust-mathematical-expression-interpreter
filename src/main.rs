...
//interpreter doesn't use list elements
//maby add interpretation to parentacies [], because I hate myself (you can make them variables_type insted of int, I hate myself), you can also addrange, I hate myself(actually you can make it a basic function its easier)


// fn main() {
//     // Create a local HashMap to store instances of CustomFunc wrapped in Arc
//     let mut local_custom_func_map: HashMap<String, Arc<CustomFunc>> = CUSTOM_FUNC_MAP.clone();
//
...
//
//     let value1 = VariableType::BasicFunWVariables(Arc::new(BasicFunWVariables::new(BASIC_FUNCTIONS.get("get_series").unwrap().clone(), vec![VariableType::Value(5.0)])));
